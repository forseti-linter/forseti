...
pub fn run(ctx: &GlobalContext, cache_path: &Path, enable_cache: bool, path: &Path, force: bool) -> Result<()> {
    let config_path = ctx.resolve_config_path(path);
    ctx.log_verbose(&format!("Using config file: {}", config_path.display()));
    
    if !config_path.exists() {
        return Err(anyhow!("No .forseti.toml found at {}. Run 'forseti init' first.", path.display()));
    }
...
fn install_engine(id: &str, cfg: &EngineCfg, cache_dir: Option<&PathBuf>, force: bool) -> Result<()> {
    println!("Installing engine: {}", id);
    
    if let Some(git_url) = &cfg.git {
...
fn install_ruleset(id: &str, cfg: &RulesetCfg, cache_dir: Option<&PathBuf>, force: bool) -> Result<()> {
...
fn install_from_git(component_type: &str, id: &str, git_url: &str, cache_dir: Option<&PathBuf>, force: bool) -> Result<()> {
    ...
    if repo_path.exists() && !force {
        ...
        if !output.status.success() {
            return Err(anyhow!("Failed to pull from git: {}", String::from_utf8_lossy(&output.stderr)));
        }
    } else {
        if repo_path.exists() {
        ...
        if !output.status.success() {
            return Err(anyhow!("Failed to clone from git: {}", String::from_utf8_lossy(&output.stderr)));
        }
    }
    ...
    if !cargo_toml.exists() {
        return Err(anyhow!("Repository does not contain a Cargo.toml file. Expected a Rust project."));
    }
    ...
    if !output.status.success() {
        return Err(anyhow!("Failed to build Rust project: {}", String::from_utf8_lossy(&output.stderr)));
    }
    ...
    if !binary_found {
        return Err(anyhow!("No executable binary found after building Rust project"));
    }
...
fn install_from_crates_io(component_type: &str, id: &str, cache_dir: Option<&PathBuf>, force: bool) -> Result<()> {
    ...
    if !output.status.success() {
        return Err(anyhow!("Failed to install from crates.io: {}", String::from_utf8_lossy(&output.stderr)));
    }
...
fn try_cargo_binstall(crate_name: &str, install_path: &Path, force: bool) -> Result<()> {
    ...
    if !output.status.success() {
        return Err(anyhow!("cargo-binstall failed: {}", String::from_utf8_lossy(&output.stderr)));
    }
